...
- 目标体重与减重进度(**阶段自适应**): 在页面顶部设置**目标体重**与**起点日期**(默认设定当天, 可改为其他日期, 用于定位"起点的 7 日均值")后, 统计卡首格变为「目标进度」并随阶段自动切换语义——**减重期**展示「已减 X kg」与「进度 X%」; 当 7 日均值进入 **目标 ±1kg** 区间(视为达标)后自动切换为**维持期**, 展示「偏离 ±X kg」与「已维持 N 天」, 使减到目标后该卡片依旧有效。所有判定与计算均基于**最大窗口均值**(默认 7 日均值)而非单日体重, 避免单日称重噪声误判。考虑到最新体重噪声大、用户更关注均值变化, 统计卡不再单独展示"最新体重"。
...
> 身高与目标体重同存于单行 `profile` 表: `height_cm`(取值范围 50-300 cm)、`target_weight`(目标体重 kg)、`target_start_date`(起点日期 yyyy-mm-dd)。三者与每日体重记录解耦, 且**各字段独立写入**(`PUT /api/profile` 仅改身高, `PUT /api/profile/target` 仅改目标, 互不覆盖)。其中 `target_weight` 须 > 0, `target_start_date` 须为合法 yyyy-mm-dd 且**不晚于今天**(后端校验, 与前端日历"禁止选未来日期"一致)。前端据身高与最新体重计算 BMI, 据目标与起点日期的均值计算减重进度/维持情况; **维持期的「已维持 N 天」按自然日(日历天)计算, 而非记录条数**, 中间有缺记的日期也能反映真实天数跨度。历史库缺少目标相关列时, 服务启动会自动补列(幂等迁移)。
