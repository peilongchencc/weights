...
- 放纵记录: 单独记录"喝酒/吃好吃的"这类放纵行为(**类型可多选**, 一次同时喝酒又吃也能记一条), 并标注**触发原因**(压力大 / 奖励自己)。约定**一天一条**(同日重复提交按日期覆盖, 选到已有记录的日期时按钮变"更新记录"并提示将覆盖), 与每日体重解耦; 列表支持**内联编辑**(修改类型 / 触发原因 / 备注)与删除, 备注过长时**悬浮显示完整内容**。页面突出展示 **「已坚持 N 天没放纵」** 横幅, 帮助靠"不要断签"的心理约束自己; 同时在趋势图上以**竖虚线 + 🍺/🍰 标记**标出放纵日, 直观对比"放纵之后体重曲线是否抬头"。

> **坚持天数口径**: N 取"**完整度过的干净自然日**"数量, 即今天与最近一次放纵日的自然日差再减 1。放纵当天与次日均记为第 0 天, 之后每多过一整天 +1(例: 06-17 放纵, 06-18 显示 0 天, 06-19 显示 1 天)。是否"**今天破功**"单独按"最近一次放纵日 === 今天"判断, 与显示天数解耦——破功当天横幅会**变红、天数归零并提示"明天重新开始"**, 而次日(天数同为 0)不会再标红, 避免误判。
