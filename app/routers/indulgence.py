...
提供"喝酒/吃好吃的"等放纵行为的录入、查询与删除接口, 帮助用户回看
触发原因(压力/奖励)并据此提醒自己。request_id 由依赖项统一注入,
用于日志追踪。
...
from app.models import IndulgenceCreate
...
async def create_indulgence(
    ...
    Args:
        payload: 含 kind / trigger / note / date 的请求体。
    ...
    record = await indulgence_crud.create_indulgence(
